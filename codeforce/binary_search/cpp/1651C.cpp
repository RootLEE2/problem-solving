...
void calcul_cost_of_case() {
  // 연결이 2개인 경우
  costPerCase.push_back(abs(a_1 - b_1) + abs(a_n - b_n));
  costPerCase.push_back(abs(a_1 - b_n) + abs(a_n - b_1));

  // 연결이 3개인 경우
  costPerCase.push_back(abs(a_1 - b_1) + minArr[0].ofN + minArr[1].ofN);
  costPerCase.push_back(abs(a_1 - b_n) + minArr[0].ofN + minArr[1].of1);
  costPerCase.push_back(abs(a_n - b_1) + minArr[0].of1 + minArr[1].ofN);
  costPerCase.push_back(abs(a_n - b_n) + minArr[0].of1 + minArr[1].of1);

  // 연결이 4개인 경우
  costPerCase.push_back(minArr[0].of1 + minArr[0].ofN + minArr[1].of1 + minArr[1].ofN);
}

void calcul_min_cost_at_edge(ll& minVal, ll grade, ll *otherRowGrade) {
  int left = 1;
  int right = n;
  ...
  while (left <= right) {
    ...
    int otherRow_i = otherRowGrade[mid];
    minVal = min(minVal, abs(grade - otherRow_i));

    if (grade == otherRow_i) return;
    if (grade < otherRow_i) {
      right = mid - 1;
    } else {
      left = mid + 1;
...
void solve() {
  ...
  sort(aGrade + 1, aGrade + n + 1);
  sort(bGrade + 1, bGrade + n + 1);
  
  calcul_min_cost_at_edge(minArr[0].of1, a_1, bGrade);
  calcul_min_cost_at_edge(minArr[0].ofN, a_n, bGrade);
  calcul_min_cost_at_edge(minArr[1].of1, b_1, aGrade);
  calcul_min_cost_at_edge(minArr[1].ofN, b_n, aGrade);
...
7. 추가 시도.
  - 각 열의 맨 끝지점에서 반대 열로의 최소 cost를 찾을 때 이분탐색을 하면 실행 시간이 더 짧지 않을까 싶었다.
  - 그런데 오히려 16ms가 더 늘어났다...
